...
Asena.addCommand({pattern: 'picanime ?(.*)', fromMe: wk, desc: pic, usage: 'animepic normal // animepic nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    ...
    await message.sendMessage(Buffer.from(image_buffer.data), MessageType.image, { caption: 'Made by Eva' })
    ...
    await message.sendMessage(Buffer.from(image_buffer_nsfw.data), MessageType.image, { caption: 'Made by Eva' })
  } else {
...
Asena.addCommand({pattern: 'gifanime ?(.*)', fromMe: wk, desc: giff, usage: 'animegif normal // animegif nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    ...
    await message.sendMessage(Buffer.from(gif_buffer.data), MessageType.video, { caption: 'Made by Eva', mimetype: Mimetype.gif })
  } else if (match[1] == 'nsfw') {
    ...
    await message.sendMessage(Buffer.from(gif_buffer_nsfw.data), MessageType.video, { caption: 'Made by Eva', mimetype: Mimetype.gif })
  } else {
    return await message.client.sendMessage(message.jid,wr_usage,MessageType.text)
  }
}));

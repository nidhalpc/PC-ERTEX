...
async function whatsAsena () {
    ...
    setInterval(async () => { 
        if (biography_var == 'true') {
            ...
            else if (conn.user.jid.startsWith('136')) { // PC/ERTEX
                const get_localized_date = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
                var utch = new Date().toLocaleDateString(config.LANG, get_localized_date)
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        ...
        console.log(chalk.bgGreen('PC/ERTEX' + afwhasena));
        await new Promise(r => setTimeout(r, 500));
        let PC/ERTEX_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '*WhatsAsena Chatbot Olarak Çalışıyor!* 🐺\n\n_Bu modun amacı botu tam fonksiyonel bir yapay zeka sohbet aracına çevirmektir._\n_Normal moda dönmek için_ *.fullPC/ERTEX off* _komutunu kullanabilirsiniz._\n\n*WhatsAsena Kullandığın İçin Teşekkürler 💌*\n    *- PC/ERTEX*' : '*WhatsAsena Working as a Chatbot! 🐺*\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fullPC/ERTEX off* _command to return to normal mode._\n\n*Thanks For Using WhatsAsena 💌*\n    *- PC/ERTEX*'
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                if (config.FULLPC/ERTEX == 'true') {
                    await conn.sendMessage(conn.user.jid, PC/ERTEX_ACTİON, MessageType.text)
                } else {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                if (error.message.includes('URL')) {
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [WHATSASENA] ⚕️*' + 
                                        ...
                                        '\n*Çözümü:* _Bir kez daha kullanırsanız düzelebilir. Hata dPC/ERTEXm ediyorsa restart atmayı deneyebilirsiniz._'
                                        , MessageType.text
                                    );
                                }
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [PC/ERTEX] --*' + 
                                    '\n*PC/ERTEX an error has occurred!*'+
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [PC/ERTEX] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
